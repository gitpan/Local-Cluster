...
Local::Cluster::Config - Cluster configuration methods

=cut
package Local::Cluster::Config;
...
use Local::Cluster::EZDB;

my %_CONF;

=head1 SYNOPSIS

 my $config = Local::Cluster::Config->new( '/config/dir' );
...
sub new
{
    ...
    {
        $_CONF{$name} = [ Local::Cluster::EZDB->new( $file ) ]
            if $name !~ /^\./o
